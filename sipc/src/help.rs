...
pub fn print_help() {
    ...
    let opts = vec![
        ...
        (
            "--zig-cc",
            "Use Zig's C compiler instead of Clang — for that extra Ziggy flavor",
        ),
        (
            "--zig-optimize <level>",
            "Zig-specific optimization level (0-3)",
        ),
        (
            "--zig-cpu-features <features>",
            "Enable specific CPU features for Zig compilation",
        ),
        (
            "--zig-no-libc",
            "Don't link libc when using Zig CC — for bare metal fun",
        ),
    ...
    let examples = vec![
        ...
        (
            "sip build foo.sip --zig-cc --zig-optimize 3",
            "Zig CC with maximum optimization — zoom zoom!",
        ),
        (
            "sip build foo.sip --zig-cc --zig-cpu-features +avx2,+fma",
            "Zig CC with specific CPU features enabled",
        ),
        (
            "sip build foo.sip --zig-cc --zig-no-libc",
            "Bare metal compilation with Zig CC — no libc crutches",
        ),
    ];
